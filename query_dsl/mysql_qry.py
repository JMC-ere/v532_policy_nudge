   ...
   where  1=1
		and (
			(
			    ...
			    or 
			    (s.start_dt between date_format(adddate(now(), {day}), '%Y%m%d000000') and date_format(adddate(now(), {day}), '%Y%m%d235959')
			    and date_format(adddate(now(), {day}), '%Y%m%d%H%i%s') < ifnull(s.end_dt, s.start_dt) )
			)
			or (
				-- 특정채널 넛지 처리
				s.nudge_id = '14'
